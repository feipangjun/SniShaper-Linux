...
type UConn struct {
	*Conn
	...
	// outerServerName is used to override the outer SNI in ECH mode
...
func UClient(conn net.Conn, config *Config, clientHelloID ClientHelloID) *UConn {
	...
	// [uTLS] Set the uconn pointer in Conn so that Conn can access UConn fields
	uconn.Conn.uconn = &uconn
	return &uconn
}
...
// SetOuterSNI sets the outer SNI for ECH mode.
// This allows overriding the public_name in ECH config.
func (uconn *UConn) SetOuterSNI(sni string) {
	uconn.outerServerName = sni
}

// GetOuterSNI returns the outer SNI if set, otherwise empty string
func (uconn *UConn) GetOuterSNI() string {
	return uconn.outerServerName
}
